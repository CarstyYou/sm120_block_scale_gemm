...
#include "cute_sm12x_gemm/sm120_bf16/builder.cuh"
...
struct SM120Bf16GemmKernel {
  ...
  using Scheduler = sm120_common::SelectedScheduler<
      KT::kGemmType, KT::kSwapAB, KT::kTileM, KT::kTileN>;
  ...
  static Params to_underlying_arguments(Arguments const& args) {
    ...
    return {tma_load_a, tma_load_b, args.ptr_D, args.grouped_layout,
            args.M, args.N, args.K, args.num_groups};
  }
  ...
  static void mma(
    ...
    Params epi_params = params;
    if constexpr (KT::kPerBatchAB) {
      epi_params.ptr_D += int64_t(tile.group) * params.M * params.N;
    }
    if constexpr (
        KT::kGemmType == sm120_common::GemmType::MGroupedMasked) {
      epi_params.M = tile.m_boundary;
    }
    if constexpr (KT::kSwapAB) {
