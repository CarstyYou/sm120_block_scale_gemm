...
struct Sm120BlockScaledStagedR2GStoreConfig {
  static_assert((kTileM == 64 || kTileM == 128) &&
                (kTileN == 64 || kTileN == 128));
  ...
  struct SharedStorageStagedR2G : cute::aligned_struct<128, _0> {
    alignas(1024) cute::ArrayEngine<
        ElementD, cute::cosize_v<SmemLayoutD>> smem_D;
  };
};

template <int kTileM, int kTileN, typename ElementD>
struct Sm120BlockScaledStagedR2GStoreConfig<
    kTileM, kTileN, ElementD, false> {
  static constexpr int StagesD = 1;
...
deduce_d_layout(int32_t M, int32_t N, int32_t L)
{
  if constexpr (kSwapAB) {
...
template <bool kSwapAB>
CUTE_DEVICE auto
make_offset_coord(int32_t m_offset)
{
  if constexpr (kSwapAB) {
...
template <bool kSwapAB>
CUTE_DEVICE auto
make_residue_coord(int32_t M, int32_t N,
                   int32_t m_block_idx, int32_t n_block_idx,
                   int32_t kTileM, int32_t kTileN)
{
  if constexpr (kSwapAB) {
    return cute::make_coord(N - kTileM * m_block_idx,
                            M - kTileN * n_block_idx);
  } else {
    return cute::make_coord(M - kTileM * m_block_idx,
                            N - kTileN * n_block_idx);
  }
}

template <typename KT, typename Accum>
CUTE_DEVICE auto
convert_accum_to_output_type(Accum const& accum)
{
  ...
  cute::for_each(cute::make_int_sequence<cute::size(epi_frg)>{}, [&](auto i) {
    epi_frg(i) = converter(accum_frg(i));
  });
  return epi;
}
    ...
    Barrier* store_empty_mbar)
{
  ...
  auto mD_mn = [&] {
    if constexpr (KT::kFlat) return mD_full;
    else                     return cute::domain_offset(make_offset_coord<KT::kSwapAB>(m_offset), mD_full);
  }();
  ...
  int32_t residue_M = [&] {
    if constexpr (KT::kFlat) return int32_t(params.M);
    else                     return m_boundary - m_offset;
  }();
  auto residue = make_residue_coord<KT::kSwapAB>(residue_M, params.N,
    ...
    Barrier* store_empty_mbar)
{
  ...
  auto sD_ = cute::make_tensor(cute::make_smem_ptr(shared_storage.tensors.store.smem_O.begin()), typename KT::R2GStoreConfig::SmemLayoutO{});
  auto sD = as_position_independent_swizzle_tensor(sD_);
  auto tiled_copy_R2S = cute::make_tiled_copy_C(typename KT::R2GStoreConfig::SmemCopyAtomR2S{}, mma);
  ...
  for (int m = 0; m < cute::size<1>(tRG_gD); ++m) {
    ...
    for (int n = 0; n < cute::size<2>(tRG_gD); ++n) {
      if (cute::get<0>(tRG_cD(0, m, n)) < cute::get<0>(residue) &&
        ...
        cute::copy(typename KT::R2GStoreConfig::GmemCopyAtomR2G{}, tRG_rD(cute::_, m, n), tRG_gD(cute::_, m, n));
      }
    }
  }
}
...
epi_r2s(
    ...
    int& epi_stage,
    uint32_t* se_phase,
    Barrier* store_full_mbar,
    Barrier* store_empty_mbar)
{
  static_assert(KT::kUseTmaStore,
      "epi_r2s requires kFlat (TMA store path)");
  ...
  auto tiled_copy_r2s = make_tiled_copy_S(
      cute::Copy_Atom<typename KT::TmaStoreConfig::CopyOpR2S, typename KT::ElementD>{}, tiled_copy_C_atom);
  auto thr_copy_r2s = tiled_copy_r2s.get_slice(thread_idx);
    ...
    Barrier* store_empty_mbar)
{
  ...
  int32_t residue_m =
      m_boundary - m_offset - m_block_idx * KT::kTileM;
  residue_m = residue_m < 0 ? 0 : residue_m;
  residue_m = residue_m > KT::kTileM ? KT::kTileM : residue_m;
  int valid_epi_m =
      (residue_m + StoreConfig::kEpiTileM - 1) /
      StoreConfig::kEpiTileM;
...
staged_r2g_store(
    Params const& params,
    SharedStorage& shared_storage,
    int32_t m_offset, int32_t m_boundary,
    int32_t m_block_idx, int32_t n_block_idx,
    ...
    Barrier* store_empty_mbar)
{
  ...
  auto mD_full = cute::make_tensor(
      cute::make_gmem_ptr(params.ptr_D),
      cute::take<0, 2>(deduce_d_layout<KT::kSwapAB>(
          params.M, params.N, 1)));
  auto mD_mn = cute::domain_offset(
      make_offset_coord<KT::kSwapAB>(m_offset), mD_full);
  auto cta_coord = cute::make_coord(m_block_idx, n_block_idx);
  auto gD = cute::local_tile(
      mD_mn,
      make_shape(cute::Int<KT::kTileM>{}, cute::Int<KT::kTileN>{}),
      cta_coord);
  ...
  auto residue = make_residue_coord<KT::kSwapAB>(
      m_boundary - m_offset, params.N,
      m_block_idx, n_block_idx, KT::kTileM, KT::kTileN);
  int32_t residue_m =
      m_boundary - m_offset - m_block_idx * KT::kTileM;
  residue_m = residue_m < 0 ? 0 : residue_m;
  residue_m = residue_m > KT::kTileM ? KT::kTileM : residue_m;
  ...
  auto sD_ = cute::make_tensor(
      cute::make_smem_ptr(shared_storage.tensors.store.smem_D.begin()),
      typename StoreConfig::SmemLayoutD{});
    ...
    for (int epi_m = 0; epi_m < valid_epi_m; ++epi_m) {
      ...
      for (int m = 0; m < cute::size<1>(tRG_gD); ++m) {
        ...
        for (int n = 0; n < cute::size<2>(tRG_gD); ++n) {
          ...
          constexpr int kVectorElements =
              decltype(cute::size<0>(tRG_gD))::value;
          auto first_coord = tRG_cD(0, m, n);
          auto last_coord = tRG_cD(kVectorElements - 1, m, n);
          tRG_pD(0) =
              cute::get<0>(first_coord) < cute::get<0>(residue) &&
              cute::get<1>(last_coord) < cute::get<1>(residue);
          cute::copy_if(
              typename StoreConfig::GmemCopyAtomR2G{},
              tRG_pD, tRG_rD(_, m, n), vector_gD);
        }
      }
    ...
    Barrier* store_full_mbar, Barrier* store_empty_mbar)
{
  ...
  auto gD_mnl = local_tile(mD_mnl, typename KT::TileShape{},
