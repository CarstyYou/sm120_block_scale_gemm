    ...
    cudaStream_t stream = 0)
{
  using Kernel = std::conditional_t<
      ...
      SM120BlockScalingMoeGemmKernel<KT>,
      SM120BlockScalingGemmKernel<KT>>;
  auto args = make_launch_args<KT, Kernel>(
      ptr_A, ptr_B, ptr_SFA, ptr_SFB, ptr_D, M, N, K, grouped_layout);
  auto problem_shape = make_shape(M, N, K, num_experts);
  launch_kernel<Kernel>(
      Kernel::to_underlying_arguments(problem_shape, args), num_sms, stream);
}
    ...
    cudaStream_t stream = 0)
{
  ...
  auto args = make_launch_args<KT, Kernel>(
      ptr_A, ptr_B, ptr_SFA, ptr_SFB, ptr_D, M, N, K, /*grouped_layout=*/nullptr);
    ...
    cudaStream_t stream = 0)
{
  using Kernel = SM120BlockScalingGemmKernel<KT>;
  auto args = make_launch_args<KT, Kernel>(
      ptr_A, ptr_B, ptr_SFA, ptr_SFB, ptr_D, max_m, N, K, masked_m);
