...
template <int TileM_, int TileN_, int TileK_, int Stages_, int GranK_,
          ...
          sm120_common::GemmType kGemmType_ = sm120_common::GemmType::Normal,
          bool kSwapAB_ = false>
struct Sm120BlockScaledSFConfig {
  ...
  static auto deduce_sfa_layout(ProblemShape const& problem_shape, int num_experts_for_sf = 0) {
    ...
    int64_t scale_m;
    if constexpr (kGemmType == sm120_common::GemmType::MGroupedContiguousWithZeroPadding && !kSwapAB) {
    ...
    int M, int N, int K, int num_experts)
{
  ...
  typename KT::SFConfig::TMA_SFA tma_load_sfa = make_tma_copy(
    SM90_TMA_LOAD{}, tensor_sfa,
    typename KT::SFConfig::SmemLayoutSFA{}(_,_,Int<0>{}),
    make_shape(shape<0>(typename KT::SFConfig::ScaleTileShape{}), shape<2>(typename KT::SFConfig::ScaleTileShape{})),
    _1{});
  ...
  typename KT::SFConfig::TMA_SFB tma_load_sfb = make_tma_copy(
    SM90_TMA_LOAD{}, tensor_sfb,
    typename KT::SFConfig::SmemLayoutSFB{}(_,_,Int<0>{}),
    make_shape(shape<1>(typename KT::SFConfig::ScaleTileShape{}), shape<2>(typename KT::SFConfig::ScaleTileShape{})),
    _1{});
...
CUTE_DEVICE auto
tma_sf_partition(
    typename KT::SFConfig::TMA_SFA const& tma_load_sfa,
    typename KT::SFConfig::TMA_SFB const& tma_load_sfb,
    int M, int N, int K, int num_experts,
    BlkCoord const& blk_coord, int32_t m_offset)
{
  ...
  auto sfa_shape = make_shape(sfa_src_M, sfb_src_N, K, sfa_src_L);
  auto mSFA_full = tma_load_sfa.get_tma_tensor(shape(KT::SFConfig::deduce_sfa_layout(sfa_shape, num_experts)));

  auto sfb_shape = make_shape(sfa_src_M, sfb_src_N, K, sfb_src_L);
  auto mSFB_full = tma_load_sfb.get_tma_tensor(shape(KT::SFConfig::deduce_sfb_layout(sfb_shape, num_experts)));
  ...
  auto mSFA = [&] {
    if constexpr (!kSwapAB && !KT::kFlat) return cute::domain_offset(make_coord(sf_m_offset, 0, 0), mSFA_full);
    else                                    return mSFA_full;
  }();
  auto mSFB = [&] {
    if constexpr (kSwapAB && !KT::kFlat) return cute::domain_offset(make_coord(sf_m_offset, 0, 0), mSFB_full);
    else                                   return mSFB_full;
  }();
