...
struct SM120BlockScaledMoeGemmKernel
  ...
  void operator()(Params const& params, char* smem_buf) {
    ...
    auto [ab_full_mbar, ab_empty_mbar, sf_full_mbar, sf_empty_mbar, store_full_mbar, store_empty_mbar]
        = BaseKernel::get_mbarriers(shared_storage);
      ...
      if (warp_idx == ab_warp_idx)
      {
        ...
        while (sched_pipeline.get_next_tile(tile)) {
          if (lane_predicate)
          {
            ...
            BaseKernel::load_ab(params, shared_storage,
      ...
      if (warp_idx == sf_warp_idx)
      {
        ...
        while (sched_pipeline.get_next_tile(tile)) {
          if (lane_predicate)
          {
            auto blk_coord = sm120_common::utils::make_blk_coord<KT::kSwapAB>(
                tile.m_block, tile.n_block, tile.group);
            BaseKernel::load_sf(params, shared_storage,
                                blk_coord, tile.m_offset,
    ...
    else
    {
      ...
      while (sched_pipeline.get_next_tile(tile)) {
        auto blk_coord = sm120_common::utils::make_blk_coord<KT::kSwapAB>(
            tile.m_block, tile.n_block, tile.group);
        BaseKernel::mma(params, shared_storage, num_sf_cycles,
