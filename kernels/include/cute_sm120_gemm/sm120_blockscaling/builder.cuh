...
#include "cute_sm120_gemm/sm120_blockscaling/sf_fp8_tma_load.cuh"
...
template <int kTileM, int kTileN, bool kUseTmaStore,
          bool kSwapAB = false, bool kUse64x32Epilogue = false>
struct SM120BlockScalingMMAConfig {
  using MMA_Atom = cute::MMA_Atom<SM120_16x8x32_TN<
      cute::float_e4m3_t, cute::float_e4m3_t, float>>;

  static_assert(kTileN >= 8 && (kTileN % 8) == 0,
                "kTileN must be >= 8 and multiple of MMA atom_N=8");
  static constexpr int kNumMathWarpN =
      kUse64x32Epilogue ? 2 : ((kTileN >= 32) ? 4 : (kTileN / 8));
  ...
  using PermMmaTileM = Int<kUse64x32Epilogue
      ? ((kTileM < 128) ? kTileM : 128)
      : ((kUseTmaStore && !kSwapAB) ? ((kTileM < 32) ? kTileM : 32)
                                     : kTileM)>;
  using PermMmaTileN = Int<kUse64x32Epilogue
      ? ((kTileN < 32) ? kTileN : 32)
      : kTileN>;
...
struct SM120BlockScalingBuilder {
  ...
  using SFConfig = SM120BlockScalingSFConfig<kTileM, kTileN, kTileK, Stages_,
                                             kGranM, kGranN, kGranK, ElementScale>;
  using SfaTmaLoadConfig =
      SM120BlockScalingSfaTmaLoadConfig<SFConfig, kUseTmaSFA>;
  static constexpr int kNumProducerThreadEvents =
      SFConfig::kNumProducerThreadEvents + (kUseTmaSFA ? 1 : 0);
  using MMAConfig = SM120BlockScalingMMAConfig<
      kTileM, kTileN, kUseTmaStore, kSwapAB, kUseStagedR2G>;
  ...
  using StagedR2GStoreConfig =
      sm120_common::Sm120BlockScaledStagedR2GStoreConfig<
          kTileM, kTileN, ElementD, kUseStagedR2G>;
  ...
  static constexpr int kNumStoreMbar =
      kUseTmaStore ? TmaStoreConfig::StagesD
                   : (kUseStagedR2G ? StagedR2GStoreConfig::StagesD : 1);
