...
#include "cute_sm120_gemm/sm120_common/ab_tma_load.cuh"
#include "cute_sm120_gemm/sm120_common/epilogue.cuh"
#include "cute_sm120_gemm/sm120_common/math.cuh"
#include "cute_sm120_gemm/sm120_common/scheduler.cuh"
...
struct SM120BlockScalingGemmKernel {
      ...
      uint32_t& sf_phase, uint32_t& store_phase)
  {
    ...
    cute::Tensor mSFA_full = make_tensor(make_gmem_ptr(params.ptr_SFA),
    ...
    cute::Tensor gSFA = local_tile(mSFA, make_tile(Int<KT::SFConfig::kTileScaleM>{}), make_coord(sfa_tile_idx, _, sfa_batch_idx));
    cute::Tensor coordSFA = local_tile(cSFA, make_tile(Int<KT::SFConfig::kTileScaleM>{}), make_coord(sfa_tile_idx, _, sfa_batch_idx));
    cute::Tensor gSFB = local_tile(mSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(sfb_tile_idx, _, sfb_batch_idx));
    cute::Tensor coordSFB = local_tile(cSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(sfb_tile_idx, _, sfb_batch_idx));
      ...
      int& epi_stage, uint32_t* se_phase)
  {
    ...
    if constexpr (KT::kUseTmaStore) {
      sm120_common::utils::epi_r2s<KT>(params, shared_storage, accum, thread_idx,
                          epi_stage, se_phase,
      ...
      uint32_t* sf_phase, int& epi_stage)
  {
    if constexpr (KT::kUseTmaStore) {
      ...
      (void)sf_full_mbar;
      (void)sf_empty_mbar;
      sm120_common::utils::tma_store<KT>(params, shared_storage,
    ...
    if (warp_idx >= KT::MMAConfig::kNumMathWarps) {
      ...
      while (scheduler.get_next_block(m_block_idx, n_block_idx)) {
        ...
        mma(params, shared_storage, k_tile_count,
            scheduler.get_m_offset(), scheduler.get_m_boundary(),
            cute::get<0>(blk_coord), cute::get<1>(blk_coord), cute::get<2>(blk_coord),
