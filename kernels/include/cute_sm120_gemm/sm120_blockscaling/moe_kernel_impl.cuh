...
struct SM120BlockScalingMoeGemmKernel : SM120BlockScalingGemmKernel<KT> {
  ...
  static_assert(!KT::kUseTmaStore,
                "the barrier init chain omits the TmaStore stages");
  ...
  using TensorStorage = std::conditional_t<
      KT::kUseStagedR2G,
      typename KT::TensorStorageStagedR2G,
      ...
      uint32_t& sf_phase, uint32_t& store_phase)
  {
    ...
    auto sSFA = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_SFA.begin()), typename KT::SFConfig::SmemLayoutTmaSFA{});
    ...
    cute::Tensor mSFB = make_tensor(make_gmem_ptr(params.ptr_SFB),
    ...
    cute::Tensor gSFB = local_tile(mSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(n_block_idx, _, expert_idx));
    cute::Tensor coordSFB = local_tile(cSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(n_block_idx, _, expert_idx));

    TiledCopy scale_copy_b = make_tiled_copy(typename KT::SFConfig::SmemCopyAtomSFB{},
    ...
    for (int32_t k_tile_idx = 0; k_tile_idx < k_tile_count; ++k_tile_idx) {
      ...
      cutlass::arch::cpasync_barrier_arrive_noinc(recast_ptr<ProducerBarrierType>(&sf_full_barrier));
      ++sf_stage;
      ...
      uint32_t& sf_phase, uint32_t& store_phase)
  {
    ...
    auto sSFA = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_SFA.begin()), typename KT::SFConfig::SmemLayoutSFA{});
    auto sSFB = make_tensor(make_smem_ptr(shared_storage.tensors.load.smem_SFB.begin()), typename KT::SFConfig::SmemLayoutSFB{});

    cute::Tensor mSFA = make_tensor(make_gmem_ptr(params.ptr_SFA),
        KT::SFConfig::deduce_sfa_layout(sfa_src_M, sfb_real_N, params.K, params.num_experts));
    cute::Tensor mSFB_full = make_tensor(make_gmem_ptr(params.ptr_SFB),
    ...
    cute::Tensor gSFA = local_tile(mSFA, make_tile(Int<KT::SFConfig::kTileScaleM>{}), make_coord(m_block_idx, _, expert_idx));
    cute::Tensor coordSFA = local_tile(cSFA, make_tile(Int<KT::SFConfig::kTileScaleM>{}), make_coord(m_block_idx, _, expert_idx));
    cute::Tensor gSFB = local_tile(mSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(n_block_idx, _, Int<0>{}));
    cute::Tensor coordSFB = local_tile(cSFB, make_tile(Int<KT::SFConfig::kTileScaleN>{}), make_coord(n_block_idx, _, Int<0>{}));

    TiledCopy scale_copy_a = make_tiled_copy(typename KT::SFConfig::SmemCopyAtomSFA{},
        Layout<Shape<Int<KT::SFConfig::kNumScaleCopyThreads>>>{}, Layout<Shape<_1>>{});
    TiledCopy scale_copy_b = make_tiled_copy(typename KT::SFConfig::SmemCopyAtomSFB{},
        Layout<Shape<Int<KT::SFConfig::kNumScaleCopyThreads>>>{}, Layout<Shape<_1>>{});
  ...
  static void load_ab(
      Params const& params, SharedStorage& shared_storage,
      BlkCoord const& blk_coord, int32_t m_offset,
      int32_t k_tile_count, int& ab_stage,
  ...
  static void mma(
      ...
      int& read_stage, uint32_t& sf_phase, uint32_t& ab_phase,
      int& epi_stage, uint32_t* se_phase)
  {
    ...
    auto tCrSFAViewAsC = make_tensor_like<typename KT::ElementScale>(tCsSFAViewAsC(_,_,_,Int<0>{}));
    auto tCrSFBViewAsC = make_tensor_like<typename KT::ElementScale>(tCsSFBViewAsC(_,_,_,Int<0>{}));
    ...
    if constexpr (KT::kUseStagedR2G) {
      sm120_common::utils::epi_staged_r2s<KT>(params, shared_storage, accum, thread_idx,
                                m_offset, m_boundary, m_block_idx,
                                epi_stage, se_phase[0],
                                store_full_mbar, store_empty_mbar);
    } else if constexpr (KT::kSwapAB) {
      sm120_common::utils::epi_pred_stg<KT>(params, accum, thread_idx,
                                m_offset, m_boundary, m_block_idx, n_block_idx,
                                store_empty_mbar);
    } else {
      sm120_common::utils::epi_pred_r2g<KT>(params, shared_storage, accum, thread_idx,
                                m_offset, m_boundary, m_block_idx, n_block_idx,
    ...
    if (warp_idx >= KT::MMAConfig::kNumMathWarps) {
      ...
      if (warp_idx == ab_warp_idx) {
        ...
        while (sched_pipeline.get_next_tile(tile)) {
          ...
          if constexpr (KT::kUseTmaSFA) {
            load_sf_tma(params, shared_storage,
                        blk_coord, tile.m_offset,
                        k_tile_count, sf_stage, sf_phase, store_phase);
          } else {
            load_sf(params, shared_storage,
                    blk_coord, tile.m_offset, tile.m_boundary,
                    k_tile_count, sf_stage, sf_phase, store_phase);
          }
        }
        ...
        if (warp_idx == store_warp_idx) {
          ...
          sm120_common::MoeWorkTile tile;
          while (sched_pipeline.get_next_tile(tile)) {
            sm120_common::utils::staged_r2g_store<KT>(params, shared_storage,
                tile.m_offset, tile.m_boundary, tile.m_block, tile.n_block, store_thread_idx,
                full_phase, store_stage, store_full_mbar, store_empty_mbar);
          }
        }
      }
    } else {
      ...
      while (sched_pipeline.get_next_tile(tile)) {
        ...
        mma(params, shared_storage, k_tile_count,
            ...
            read_stage, sf_phase, ab_phase, epi_stage, se_phase);
      }
    }
  }
};
