...
template <int kTileM, int kTileN, int kTileK, int kStages,
          int kGranM, int kGranN, int kGranK, typename ElementScale_>
struct SM120BlockScalingSFConfig {
  ...
  using SmemLayoutTmaSFA = Layout<
      Shape<Int<kTileScaleM>, Int<kTileScaleK>, Int<SF_Stages>>>;
  ...
  using SmemLayoutSFAViewAsC = Layout<
      Shape<Shape<Int<kGranM>, Int<kTileScaleM>>, Int<kTileN>, Int<SF_Stages>>,
      Stride<Stride<_0, _1>, _0, Int<kTileScaleM>>>;

  using SmemLayoutSFBViewAsC = Layout<
      Shape<Int<kTileM>, Shape<Int<kGranN>, Int<kTileScaleN>>, Int<SF_Stages>>,
  ...
  static auto get_tma_aligned_size(const int& x) {
    ...
    CUTE_STATIC_ASSERT(
        kNumTMAAlignmentBytes % sizeof(ElementScale) == 0,
  ...
  static auto deduce_sfa_layout(int M, int N, int K, int L) {
    (void)N;
    int64_t scale_m = static_cast<int64_t>(
        sm120_common::math::ceil_div(M, kGranM));
    int64_t scale_k = static_cast<int64_t>(
        sm120_common::math::ceil_div(K, kGranK));
  ...
  static auto deduce_tma_sfa_layout(int M, int N, int K, int L) {
    (void)N;
    int64_t scale_m = static_cast<int64_t>(get_tma_aligned_size(
        sm120_common::math::ceil_div(M, kGranM)));
    int64_t scale_k = static_cast<int64_t>(
        sm120_common::math::ceil_div(K, kGranK));
    return make_layout(make_shape(scale_m, scale_k, int64_t(L)),
  ...
  static auto deduce_sfb_layout(int M, int N, int K, int L) {
    (void)M;
    int64_t scale_n = static_cast<int64_t>(
        sm120_common::math::ceil_div(N, kGranN));
    int64_t scale_k = static_cast<int64_t>(
        sm120_common::math::ceil_div(K, kGranK));
...
struct SM120BlockScalingSfaTmaLoadConfig {
  ...
  using TMA_SFA = decltype(make_tma_copy(
      ...
      make_shape(
          shape<0>(typename SFConfig::ScaleTileShape{}),
  ...
  static constexpr uint32_t TmaTransactionBytesSFA = static_cast<uint32_t>(
      cutlass::bits_to_bytes(
          cosize(take<0, 2>(typename SFConfig::SmemLayoutTmaSFA{})) *
...
static auto make_sfa_tma_descriptor(
    ...
    int M, int N, int K, int num_experts)
{
  ...
  auto sfa_layout = KT::SFConfig::deduce_tma_sfa_layout(
      sfa_src_M, N, K, sfa_src_L);
  ...
  typename KT::SfaTmaLoadConfig::TMA_SFA tma_load_sfa = make_tma_copy(
      ...
      typename KT::SFConfig::SmemLayoutTmaSFA{}(_, _, Int<0>{}),
      make_shape(
          shape<0>(typename KT::SFConfig::ScaleTileShape{}),
...
CUTE_DEVICE auto tma_sfa_partition(
    typename KT::SfaTmaLoadConfig::TMA_SFA const& tma_load_sfa,
    int M, int N, int K, int num_experts,
    BlkCoord const& blk_coord, int32_t m_offset)
{
  ...
  if constexpr (
      ...
      sm120_common::GemmType::MGroupedContiguousWithZeroPadding) {
    sfa_src_M = sm120_common::math::compute_padded_offset(M, num_experts);
    sf_m_offset = sm120_common::math::compute_padded_offset(
        m_offset, expert_idx);
  }
  ...
  auto mSFA = [&] {
    if constexpr (!KT::kFlat) {
      return domain_offset(
          make_coord(sf_m_offset / KT::kGranM, 0, 0), mSFA_full);
    } else {
      return mSFA_full;
    }
  }();
  auto gSFA_mkl = local_tile(
      mSFA, typename KT::SFConfig::ScaleTileShape{}, make_coord(_, _, _),
      Step<_1, X, _1>{});
