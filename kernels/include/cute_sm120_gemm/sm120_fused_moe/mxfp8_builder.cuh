...
template <int TileM_ = 128, int TileN_ = 64, int TileK_ = 128, int Stages_ = 2,
          int GranK_ = 128, bool SwapAB_ = false, bool UseStagedR2G_ = false>
struct SM120BlockScaledFusedMoeBuilder
  ...
  using Base = SM120BlockScaledBuilder<TileM_, TileN_, TileK_, Stages_, GranK_,
                                       sm120_common::GemmType::MGroupedContiguousWithZeroPadding, SwapAB_>;
  ...
  using StagedR2GStoreConfig = sm120_common::Sm120BlockScaledStagedR2GStoreConfig<
      TileM_, TileN_, typename Base::ElementD, kUseStagedR2G>;
  ...
  struct SharedStorageLoadDefault : cute::aligned_struct<128, _0> {
    ...
    alignas(1024) cute::ArrayEngine<ElementB, cute::cosize_v<typename ABLoadConfig::SmemLayoutB>> smem_B_up;
    alignas(1024) cute::ArrayEngine<ElementB, cute::cosize_v<typename ABLoadConfig::SmemLayoutB>> smem_B_gate;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFA>> smem_SFA;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFB>> smem_SFB_up;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFB>> smem_SFB_gate;
  };

  struct SharedStorageLoadSwapAB : cute::aligned_struct<128, _0> {
    alignas(1024) cute::ArrayEngine<ElementA, cute::cosize_v<typename ABLoadConfig::SmemLayoutA>> smem_A_up;
    alignas(1024) cute::ArrayEngine<ElementA, cute::cosize_v<typename ABLoadConfig::SmemLayoutA>> smem_A_gate;
    alignas(1024) cute::ArrayEngine<ElementB, cute::cosize_v<typename ABLoadConfig::SmemLayoutB>> smem_B;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFA>> smem_SFA_up;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFA>> smem_SFA_gate;
    cute::ArrayEngine<typename SFConfig::ElementSFLoad, cute::cosize_v<typename SFConfig::SmemLayoutSFB>> smem_SFB;
  };

  using SharedStorageLoad = std::conditional_t<SwapAB_, SharedStorageLoadSwapAB, SharedStorageLoadDefault>;
  ...
  using TensorStorageUnion = std::conditional_t<kUseStagedR2G, TensorStorageSplit, TensorStorageUnionDefault>;
};
