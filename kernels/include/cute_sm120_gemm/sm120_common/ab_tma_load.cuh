...
static auto
make_ab_tma_descriptors(
    typename KT::ElementA const* ptr_A, typename KT::ABLoadConfig::StrideA dA,
    typename KT::ElementB const* ptr_B, typename KT::ABLoadConfig::StrideB dB,
    int M, int N, int K, int num_experts)
{
  ...
  auto tensor_A = make_tensor(make_gmem_ptr(a_src),
      make_layout(make_shape(a_src_M, K, a_src_L), da_src));
  typename KT::ABLoadConfig::TMA_A tma_load_a = make_tma_copy(
    SM90_TMA_LOAD{}, tensor_A,
    typename KT::ABLoadConfig::SmemLayoutA{}(_,_,Int<0>{}),
    make_shape(shape<0>(typename KT::TileShape{}), shape<2>(typename KT::TileShape{})),
    _1{});

  auto tensor_B = make_tensor(make_gmem_ptr(b_src),
      make_layout(make_shape(b_src_N, K, b_src_L), db_src));
  typename KT::ABLoadConfig::TMA_B tma_load_b = make_tma_copy(
    SM90_TMA_LOAD{}, tensor_B,
    typename KT::ABLoadConfig::SmemLayoutB{}(_,_,Int<0>{}),
...
CUTE_DEVICE auto
tma_ab_partition(
    typename KT::ABLoadConfig::TMA_A const& tma_load_a, typename KT::ABLoadConfig::TMA_B const& tma_load_b,
    int M, int N, int K, int num_experts,
    BlkCoord const& blk_coord, int32_t m_offset)
{
  ...
  auto mA = [&] {
    if constexpr (!kSwapAB && !KT::kFlat) return cute::domain_offset(make_coord(m_offset, 0, 0), mA_full);
    else                                    return mA_full;
  }();
  auto mB = [&] {
    if constexpr (kSwapAB && !KT::kFlat) return cute::domain_offset(make_coord(m_offset, 0, 0), mB_full);
    else                                   return mB_full;
  }();
